...
void *TryResolveThisLibrary()
{
	...
	GetModuleHandleExW(0, reinterpret_cast<LPCWSTR>(&TryResolveThisLibrary), &moduleHandle);
	return moduleHandle;
}
...
void *CustomLibraryResolverCallback()
{
	...
	if (!moduleHandle)
	{
		...
		{
			if (_wcsicmp(targetLibraryName, L"dlssg_to_fsr3_amd_is_better.dll") == 0)
			{
				EnableAggressiveHooking = false;
				moduleHandle = GetModuleHandleW(nullptr);
			}
			else if (_wcsicmp(targetLibraryName, L"nvngx.dll") == 0)
			{
				// Check the registry
				EnableAggressiveHooking = false;
