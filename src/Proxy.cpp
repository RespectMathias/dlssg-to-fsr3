...
constexpr wchar_t TargetDlssgDll[] = L"nvngx_dlssg.dll";
...
std::optional<HMODULE> LoadReplacementLibrary()
{
	...
	return reinterpret_cast<HMODULE>(TryResolveThisLibrary());
}
...
bool TryRemapModule(const wchar_t *Path, HMODULE *ModuleHandle)
{
	...
	if (remappedPath.ends_with(TargetDlssgDll))
	{
		const auto newHandle = LoadReplacementLibrary();
...
void InitializeProxy(HINSTANCE hInstDLL)
{
	wchar_t modulePath[2048];
	const auto moduleName = Util::GetModulePath(modulePath, false, hInstDLL);
	if (moduleName && _wcsicmp(moduleName, L"dlssg_to_fsr3_amd_is_better.dll") == 0)
		return;
