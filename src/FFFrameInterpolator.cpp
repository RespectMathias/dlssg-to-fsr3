...
FrameGenerationStatus FFFrameInterpolator::Dispatch(void *CommandList, NGXInstanceParameters *NGXParameters)
{
	...
	FfxApiResource gameBackBufferResource = {};
	FfxApiResource gameRealOutputResource = {};

	const auto dispatchStatus = [&]() -> FrameGenerationStatus
	{
		const bool enableInterpolation = NGXParameters->GetUIntOrDefault("DLSSG.EnableInterp", 0) != 0;

		LoadTextureFromNGXParameters(NGXParameters, "DLSSG.Backbuffer", &gameBackBufferResource, FFX_API_RESOURCE_STATE_COMPUTE_READ);
		LoadTextureFromNGXParameters(NGXParameters, "DLSSG.OutputReal", &gameRealOutputResource, FFX_API_RESOURCE_STATE_UNORDERED_ACCESS);
		...
		if (auto status = m_FrameInterpolatorContext->Dispatch(fsrFiDispatchDesc); status != FrameGenerationStatus::Success)
			return status;
	...
	if ((dispatchStatus == FrameGenerationStatus::Success || dispatchStatus == FrameGenerationStatus::FlushRequired) && gameBackBufferResource.resource)
	{
		if (gameRealOutputResource.resource)
			CopyTexture(GetActiveCommandList(), &gameRealOutputResource, &gameBackBufferResource);

		if (dispatchStatus == FrameGenerationStatus::FlushRequired)
		{
			FfxApiResource outputInterp = {};

			if (LoadTextureFromNGXParameters(NGXParameters, "DLSSG.OutputInterpolated", &outputInterp, FFX_API_RESOURCE_STATE_UNORDERED_ACCESS))
...
void FFFrameInterpolator::Create(ID3D12Device *Device, NGXInstanceParameters *NGXParameters)
{
	if (NGXParameters)
	{
		void *allocCallback = nullptr;
		void *releaseCallback = nullptr;
		NGXParameters->GetVoidPointer("ResourceAllocCallback", &allocCallback);
		NGXParameters->GetVoidPointer("ResourceReleaseCallback", &releaseCallback);
		if (allocCallback || releaseCallback)
		{
			static const bool once = []()
			{
				spdlog::warn("Stock FidelityFX API does not expose GPU resource allocation callbacks; NGX callbacks will not be used.");
				return true;
			}();
		}
	}

	m_FrameInterpolatorContext.emplace(Device, m_SwapchainWidth, m_SwapchainHeight);
}
...
bool FFFrameInterpolator::CalculateResourceDimensions(NGXInstanceParameters *NGXParameters)
{
	...
	FfxApiResource temp = {};
	{
		...
		if (width == 0 || height == 0)
		{
			LoadTextureFromNGXParameters(NGXParameters, "DLSSG.Depth", &temp, FFX_API_RESOURCE_STATE_COPY_DEST);
...
#if 0
	if (LoadTextureFromNGXParameters(NGXParameters, "DLSSG.HUDLess", &temp, FFX_API_RESOURCE_STATE_COPY_DEST))
	{
	...
	NGXInstanceParameters *NGXParameters)
{
	...
	LoadTextureFromNGXParameters(NGXParameters, "DLSSG.HUDLess", &desc.InputHUDLessColorBuffer, FFX_API_RESOURCE_STATE_COPY_DEST);

	if (!LoadTextureFromNGXParameters(NGXParameters, "DLSSG.Backbuffer", &desc.InputColorBuffer, FFX_API_RESOURCE_STATE_COMPUTE_READ) &&
	...
	if (!LoadTextureFromNGXParameters(NGXParameters, "DLSSG.Depth", &desc.InputDepth, FFX_API_RESOURCE_STATE_COPY_DEST))
		return false;

	if (!LoadTextureFromNGXParameters(NGXParameters, "DLSSG.MVecs", &desc.InputMotionVectors, FFX_API_RESOURCE_STATE_COPY_DEST))
		return false;
	...
	FfxApiDimensions2D mvecExtents = {
	...
	if (NGXParameters->GetUIntOrDefault("DLSSG.MvecDilated", 0) != 0)
	{
		static const bool once = []()
		{
			spdlog::warn("Stock FidelityFX API does not expose dilated motion-vector input; motion vectors will be processed normally.");
			return true;
		}();
	}
